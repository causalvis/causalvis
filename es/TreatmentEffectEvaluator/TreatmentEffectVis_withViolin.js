...
export var TreatmentEffectVisViolin = function TreatmentEffectVisViolin(_ref) {
  ...
  useEffect(function () {
    ...
    if (!stratifyBy) {
      ...
      var _adjustedExtent = [effectExtent[0] - _adjustedBandwidth, effectExtent[1] + _adjustedBandwidth];
  ...
  useEffect(function () {
    ...
    if (stratifyBy === '') {
      ...
      var yScale = d3.scaleLinear().domain(effectExtent).range([layout.height - layout.marginBottom, layout.margin]);
      var _computedBandwidth = xScale.bandwidth();
      var _customBandwidth = layout.width / 8;
      var binScales = [];
      ...
      var _loop2 = function _loop2() {
        ...
        var newScale = d3.scaleLinear().range([0, _computedBandwidth]).domain([-maxNum, maxNum]);
        binScales.push({
          "scale": newScale,
          "len": totalLength
        });
      ...
      var _loop = function _loop(i) {
        ...
        svgElement.select("#violin").selectAll(".point" + i).data(binEffects).join("circle").attr("class", "point" + i).attr("cx", function (d) {
          return xScale(i) + _computedBandwidth / 4 + (Math.random() - 0.5) * jitter;
        }).attr("cy", function (d) {
          return yScale(d);
        }).attr("r", 3).attr("opacity", 0.2).attr("fill", colorMap[i]);
        svgElement.select("#IQR").selectAll(".boxplot" + i).data([binStats]).join("rect").attr("class", "boxplot" + i).attr("x", xScale(i) + _computedBandwidth / 4 - 15).attr("y", function (d) {
          return yScale(d.Q3);
        }).attr("width", 30).attr("height", function (d) {
          return yScale(d.Q1) - yScale(d.Q3);
        }).attr("fill", "none").attr('stroke', "black");
        svgElement.select("#IQR").selectAll(".boxmedian" + i).data([binStats]).join("line").attr("class", "boxmedian" + i).attr("x1", xScale(i) + _computedBandwidth / 4 - 15).attr("y1", function (d) {
          return yScale(d.Q2);
        }).attr("x2", xScale(i) + _computedBandwidth / 4 + 15).attr("y2", function (d) {
          return yScale(d.Q2);
        }).attr("fill", "none").attr('stroke', "black");
        svgElement.select("#IQR").selectAll(".whiskerTop" + i).data([binStats]).join("line").attr("class", "whiskerTop" + i).attr("x1", xScale(i) + _computedBandwidth / 4).attr("y1", function (d) {
          return yScale(d.IQRMax);
        }).attr("x2", xScale(i) + _computedBandwidth / 4).attr("y2", function (d) {
          return yScale(d.Q3);
        }).attr("fill", "none").attr('stroke', "black");
        svgElement.select("#IQR").selectAll(".whiskerBottom" + i).data([binStats]).join("line").attr("class", "whiskerBottom" + i).attr("x1", xScale(i) + _computedBandwidth / 4).attr("y1", function (d) {
          return yScale(d.Q1);
        }).attr("x2", xScale(i) + _computedBandwidth / 4).attr("y2", function (d) {
          return yScale(d.IQRMin);
        }).attr("fill", "none").attr('stroke', "black");
        svgElement.select("#IQR").selectAll(".topCap" + i).data([binStats]).join("line").attr("class", "topCap" + i).attr("x1", xScale(i) + _computedBandwidth / 4 - 5).attr("y1", function (d) {
          return yScale(d.IQRMax);
        }).attr("x2", xScale(i) + _computedBandwidth / 4 + 5).attr("y2", function (d) {
          return yScale(d.IQRMax);
        }).attr("fill", "none").attr('stroke', "black");
        svgElement.select("#IQR").selectAll(".bottomCap" + i).data([binStats]).join("line").attr("class", "bottomCap" + i).attr("x1", xScale(i) + _computedBandwidth / 4 - 5).attr("y1", function (d) {
          return yScale(d.IQRMin);
        }).attr("x2", xScale(i) + _computedBandwidth / 4 + 5).attr("y2", function (d) {
          return yScale(d.IQRMin);
        }).attr("fill", "none").attr('stroke', "black");
      ...
      }).attr("r", 3).attr("opacity", 0.2).attr("fill", "#698fb8").attr("cursor", "pointer");
      ...
      var _xAxis2 = svgElement.select('#x-axis').attr('transform', "translate(0, " + (layout.height - layout.marginBottom / 2) + ")").call(d3.axisBottom(xScale).tickSize(3).ticks(5));
    }
