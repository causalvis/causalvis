import React, {useRef, useState, useEffect} from 'react'
import * as d3 from 'd3';

export const SMDVis = ({layout = {"height": 500, "width": 600, "margin": 50, "marginLeft": 150}, SMDDataset = []}) => {

  // console.log("in vis", SMDDataset);

  const ref = useRef('svgSMD');

  let svg = d3.select(ref.current)

  let svgElement = svg.select("g");

  var xScale = d3.scaleLinear()
          .domain([Math.min(d3.min(SMDDataset, d => d.unadjusted), d3.min(SMDDataset, d => d.adjusted)), Math.max(d3.max(SMDDataset, d => d.unadjusted), d3.max(SMDDataset, d => d.adjusted))])
          .range([layout.marginLeft, layout.width - layout.margin])

  var yScale = d3.scaleBand()
          .domain(SMDDataset.map(d => d.covariate))
          .range([layout.height - layout.margin, layout.margin])

  svgElement.select("#adjusted")
    .selectAll(".adjustedSMD")
    .data(SMDDataset)
    .join("circle")
    .attr("class", "adjustedSMD")
    .attr("cx", d => xScale(d.adjusted))
    .attr("cy", d => yScale(d.covariate) + yScale.bandwidth() / 2)
    .attr("r", 3)
    .attr("fill", "black")
    .attr("stroke", "black")

  svgElement.select("#unadjusted")
    .selectAll(".unadjustedSMD")
    .data(SMDDataset)
    .join("circle")
    .attr("class", "adjustedSMD")
    .attr("cx", d => xScale(d.unadjusted))
    .attr("cy", d => yScale(d.covariate) + yScale.bandwidth() / 2)
    .attr("r", 3)
    .attr("fill", "white")
    .attr("stroke", "black")

  svgElement.select("#diff")
    .selectAll(".diffLine")
    .data(SMDDataset)
    .join("line")
    .attr("class", "diffLine")
    .attr("x1", d => xScale(d.unadjusted))
    .attr("y1", d => yScale(d.covariate) + yScale.bandwidth() / 2)
    .attr("x2", d => xScale(d.adjusted))
    .attr("y2", d => yScale(d.covariate) + yScale.bandwidth() / 2)
    .attr("stroke", "black")
    .attr("stroke-dasharray", "2")

  svgElement.select("#threshold")
    .selectAll(".thresholdLine")
    .data([0.1])
    .join("line")
    .attr("class", "thresholdLine")
    .attr("x1", d => xScale(d))
    .attr("y1", layout.margin - 20)
    .attr("x2", d => xScale(d))
    .attr("y2", layout.height - layout.margin + 20)
    .attr("stroke", "black")
    .attr("stroke-dasharray", "2")

  svgElement.select("#threshold")
    .selectAll(".thresholdText")
    .data([0.1])
    .join("text")
    .attr("class", "thresholdText")
    .attr("x", d => xScale(d) + 5)
    .attr("y", layout.margin - 20)
    .text(d => d)
    .attr("font-family", "sans-serif")
    .attr("font-size", 10)
    .attr("alignment-baseline", "hanging")

  svgElement.select("#legend")
    .selectAll(".legend")
    .data(["adjusted", "unadjusted"])
    .join("circle")
    .attr("class", "legend")
    .attr("cx", layout.width - 2*layout.margin)
    .attr("cy", (d, i) => layout.height - 2*layout.margin + i * 10)
    .attr("r", 3)
    .attr("fill", d => d === "adjusted" ? "black" : "white")
    .attr("stroke", "black")

  svgElement.select("#legend")
    .selectAll(".legendText")
    .data(['adjusted', 'unadjusted'])
    .join("text")
    .attr("class", "legendText")
    .attr("x", layout.width - 2*layout.margin + 10)
    .attr("y", (d, i) => layout.height - 2*layout.margin + i * 10)
    .text(d => d)
    .attr("alignment-baseline", "middle")
    .attr("font-family", "sans-serif")
    .attr("font-size", 10)

  svgElement.select('#x-axis')
          .attr('transform', `translate(0, ${layout.height - layout.margin})`)
          .call(d3.axisBottom(xScale).tickSize(3).ticks(5))

  svgElement.select('#y-axis')
          .attr('transform', `translate(${layout.marginLeft}, 0)`)
          .call(d3.axisLeft(yScale).tickSize(3).ticks(5))

  return (
    <div>
      <svg width={layout.width} height={layout.height} ref={ref} id="svgSMD">
        <g>
          <g id="diff" />
          <g id="threshold" />
          <g id="unadjusted" />
          <g id="adjusted" />
          <g id="x-axis" />
          <g id="y-axis" />
          <g id="legend" />
        </g>
      </svg>
    </div>
  )
}